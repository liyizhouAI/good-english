...
import { addMaterial, getAllMaterials, deleteMaterial } from '@/lib/db/materials';
...
import {
  FileText, Link as LinkIcon, Loader2, Sparkles, Trash2,
...
export default function ImportPage() {
  ...
  async function handleExtract() {
    ...
    try {
      ...
      setSelectedWords(new Set(data.words?.map((_: unknown, i: number) => i) || []));
      setSelectedPatterns(new Set(data.patterns?.map((_: unknown, i: number) => i) || []));
      setStep('review');
    } catch {
  ...
  async function handleSave() {
    ...
    const patternIds = patternsToSave.length > 0 ? await addPatterns(patternsToSave) : [];
    ...
    <div className="max-w-3xl mx-auto">
      ...
      {showLibrary && materials.length > 0 && (
        <div className="mb-6 space-y-2">
          {materials.map(mat => (
            <div key={mat.id} className="flex items-center justify-between rounded-lg border border-[var(--border)] bg-[var(--card)] px-4 py-3">
              <div className="flex-1 min-w-0">
                <p className="text-sm truncate">{mat.title}</p>
                <p className="text-xs text-[var(--muted-foreground)]">
                  {mat.extractedWordIds.length} 词 · {mat.extractedPatternIds.length} 句型 · {new Date(mat.createdAt).toLocaleDateString()}
                </p>
              </div>
              <button
      ...
      {step === 'extracting' && (
        <div className="flex flex-col items-center justify-center py-16 gap-4">
          ...
          <p className="text-sm text-[var(--muted-foreground)]">AI 正在分析内容，提取词汇和句型...</p>
        </div>
      )}
      ...
      {step === 'review' && extraction && (
        <div className="space-y-6">
          ...
          {extraction.words?.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold">提取的词汇 ({selectedWords.size}/{extraction.words.length})</h3>
                <button
                  onClick={() => {
                    if (selectedWords.size === extraction.words.length) {
                      setSelectedWords(new Set());
                    } else {
                      setSelectedWords(new Set(extraction.words.map((_, i) => i)));
                    }
                  }}
                  className="text-xs text-[var(--primary)] hover:underline"
                >
                  {selectedWords.size === extraction.words.length ? '取消全选' : '全选'}
                </button>
              </div>
                ...
                {extraction.words.map((word, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-[var(--muted-foreground)] mt-1 italic">{word.exampleSentence}</p>
          ...
          {extraction.patterns?.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold">提取的句型 ({selectedPatterns.size}/{extraction.patterns.length})</h3>
                <button
                  onClick={() => {
                    if (selectedPatterns.size === extraction.patterns.length) {
                      setSelectedPatterns(new Set());
                    } else {
                      setSelectedPatterns(new Set(extraction.patterns.map((_, i) => i)));
                    }
                  }}
                  className="text-xs text-[var(--primary)] hover:underline"
                >
                  {selectedPatterns.size === extraction.patterns.length ? '取消全选' : '全选'}
                </button>
              </div>
              <div className="space-y-2">
                {extraction.patterns.map((pattern, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium font-mono text-sm">{pattern.pattern}</p>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              onClick={() => { setStep('input'); setExtraction(null); }}
      ...
      {step === 'saved' && (
        <div className="flex flex-col items-center justify-center py-16 gap-4">
          ...
          <p className="text-sm text-[var(--muted-foreground)]">保存成功！词汇和句型已加入学习库</p>
        </div>
      )}
    </div>
  );
