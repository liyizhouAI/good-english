...
import {
  MessageSquare, BookOpen, FileText, Import,
  Settings, LayoutDashboard, ChevronLeft, ChevronRight, Menu, X,
} from 'lucide-react';
  ...
  return (
    <>
      ...
      <aside className={cn(
        ...
        {/* ── Sidebar header: slogan (hidden when desktop-collapsed) ── */}
        <div className={cn(
          'border-b border-[var(--border)]',
          collapsed ? 'md:hidden' : 'flex items-start gap-2 px-5 py-4',
        )}>
          <p className="flex-1 text-[11px] leading-[1.6] text-[var(--muted-foreground)]">
            好英语，让你像播客嘉宾一样侃侃而谈，流利、深刻而温暖人心。
          </p>
          {/* Mobile close button */}
          <button
            className="md:hidden shrink-0 p-1 rounded hover:bg-[var(--secondary)] text-[var(--muted-foreground)]"
            onClick={() => setMobileOpen(false)}
          >
            <X className="h-4 w-4" />
          </button>
        ...
        {/* ── Navigation ── */}
        <nav className={cn('flex-1 px-2 py-3 space-y-0.5', collapsed && 'pt-4')}>
          {NAV_ITEMS.map(item => {
